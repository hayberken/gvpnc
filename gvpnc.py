...
def on_RunNow(*args):
	...
	config = file("/etc/vpnc/gvpnc.conf", "w+")
	print >>config, "# AUTOGENERATED BY gvpnc"
	print >>config, "IPSec gateway", sServer
	print >>config, "IPSec ID", sGroup
	print >>config, "IPSec secret", sGroupPass
	print >>config, "Xauth username", sUser
	print >>config, "Xauth password", sPass
	config.close()
	HideWindow('run_command')
	Refresh()
	Refresh()
	sResult = subprocess.call(["vpnc", "/etc/vpnc/gvpnc.conf"])
...
def on_Stop(*args):
	sServer = GetWidgetText('run_server')
	sResult = subprocess.call(["vpnc-disconnect"])
#	if not 'vpnc: ' in sResult:
	Status('Server: ' + sServer + ' Disconnected')
	SetWidgetText('txtResult', sResult)
